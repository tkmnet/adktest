    ...
    public List<EntityID> getPath(int time, EntityID startID, EntityID goalID) {
        ...
        while(open.size() != 0) {
            ...
            for(EntityID neighbourID : current.getNeighbours()) {
                ...
                double neighbourDistance = currentDistance + graph.getEdge(currentID, neighbourID).getDistance();
                // double neighbourDistance = currentDistance + Math.sqrt(Math.pow(((Area)world.getEntity(currentID)).getX() - ((Area)world.getEntity(neighbourID)).getX(),2) + Math.pow(((Area)world.getEntity(currentID)).getY() - ((Area)world.getEntity(neighbourID)).getY(),2));
